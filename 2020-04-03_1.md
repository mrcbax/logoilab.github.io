...
### This page is a work in progress, I wanted it published while I write it though. Think of it as a livestreamed publication.
...
Zoom has multiple launcher binaries. Let's take a look at the `.desktop` file to see what a launcher gets called when a user chooses it in a menu.
...
The one-liner is executed via `popen()`. The client then parses the output of the executed one-liner and stores it in a `long int` using `strtol()`. 
